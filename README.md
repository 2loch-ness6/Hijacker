...
This application requires an **ARM64/ARM** android device with an internal or external wireless adapter that supports **Monitor Mode**. 

## Recommended Setup

The recommended way to use Hijacker is with **Kali NetHunter**, which provides:
- Pre-configured Kali Linux chroot environment
- Support for modern Qualcomm chipsets with monitor mode
- Enhanced wireless driver support
- Better integration with pentesting tools

An alternative would be to use an external USB wireless adapter that supports **monitor mode** in Android with an OTG cable.

The required tools are included for **ARM64** and **ARM** devices.

Root access is necessary, as these tools need root to work.
...
* **Reaver WPS** cracking (pixie-dust attack using NetHunter chroot recommended)
...
* Works seamlessly with Kali NetHunter for enhanced capabilities
* Optimized for modern ARM64 devices and Qualcomm chipsets with monitor mode support
...
* you are rooted
* you have monitor mode support (via NetHunter, custom firmware, or external adapter)

### NetHunter Installation (Recommended)
For the best experience, install [Kali NetHunter](https://www.kali.org/docs/nethunter/) on your device. NetHunter provides:
- Full Kali Linux environment via chroot
- Enhanced wireless driver support
- Support for modern Qualcomm chipsets
- Pre-configured monitor mode scripts

### Standalone Installation
#### Download the latest version [here](https://github.com/chrisk44/Hijacker/releases).

When you run Hijacker for the first time, make sure you have enabled your WiFi and configured monitor mode commands in settings if needed. For NetHunter users, the app will automatically detect and use NetHunter's capabilities.
...
### Device
This app is provided as-is. While it no longer modifies system firmware files, improper use of wireless penetration testing tools can cause issues. Use NetHunter or compatible monitor mode solutions at your own risk. I am not responsible for any damage caused to you, your device, your house, your cat, or anyone and anything else, by this software.
